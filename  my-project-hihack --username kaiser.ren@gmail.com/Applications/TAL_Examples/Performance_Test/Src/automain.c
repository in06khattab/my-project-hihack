...
static uint32_t number_test_frames = 10000;
...
static void start_carrier_test(void);
//static retval_t set_lwpwr_mode(void);
...
#endif
static void appTimer2_callback(void);

#if (PAL_GENERIC_TYPE==ARM7)
#pragma location="RomBootFlag"
__root const uint32_t		Bf_Appflag = 0xa5a55a5al;
#endif
...
int main(void)
{
    ...
    if (tal_init() != MAC_SUCCESS)
    {
	 ...
	 /*Enable transciver's extended feature */
	 pal_trx_reg_write(RG_TRX_CTRL_1, (pal_trx_reg_read(RG_TRX_CTRL_1) | 0x80));
#endif
...
#if PAL_TYPE == AT91SAM7X256	
	 AT91C_BASE_PIOA->PIO_PPUDR = AT91C_PA5_RXD1 | AT91C_PA6_TXD1;
#endif
    ...
    configure_frame_sending();
	 ...
	 //printf("MUCSR = 0x%02X\r\n", MCUSR);
	 printf("Welcome to RAEMeshII RangeTest\r\n");
#if PAL_TYPE == ATMEGA1281
	 printf("Platform: ATMEGA1281\r\n");
#elif PAL_TYPE == AT91SAM7X256
	 printf("Platform: AT91SAM7X256\r\n");
#endif
	
#if TAL_TYPE==AT86RF212	
	 printf("Radio: AT86RF212\r\n");
#elif TAL_TYPE==AT86RF230B	
	 printf("Radio: AT86RF230B\r\n");
#elif TAL_TYPE==AT86RF231
	 printf("Radio: AT86RF231\r\n");
#else
	 printf("Radio: unknow\r\n");
#endif
...
#elif defined (AT86RF230B) || defined(AT86RF231) || defined(AT86RF230)
   static uint8_t channel = 11;
#endif
	
	tfa_continuous_tx_stop();
	
	pal_timer_start(T_APP_TIMER2,
                         1000000,
                         TIMEOUT_RELATIVE,
                         (FUNC_PTR)appTimer2_callback,
                         NULL);
	...
	printf("appTimer2_callback ch: %d\r\n", channel);
}
...
void tal_rx_frame_cb(frame_info_t *frame)
{
    ...
    //if (op_mode == PROMISCUOUS_OP_MODE)
    {
        ...
        printf("Rx: %d ", number_rx_frames);
        //for (i = 0; i < (frame->mpdu[0]); i++)    /* Including FCS & LQI */
        //{
        //    sprintf(ascii, "%c", frame->mpdu[i]);
       //     printf(ascii);
       // }
...
#error "unknown PAL_GENERIC_TYPE";
    ...
    printf("(Z) : start carrier wave test \r\n");
    ...
    switch (input)
    {
	 	  case 'Z': start_carrier_test(); break;
...
/**
 * @brief start carrier test
 */
static void start_carrier_test(void)
{
  	 char c;
#if (TAL_TYPE == AT86RF212)
	 uint8_t temp_var = 10, channel = 6;
#else
	 uint8_t temp_var = 3, channel = 11;
#endif
	 void (*reset)(void) = 0;
	
	 printf("\r\nPress any key to end of test \r\n");
#if (TAL_TYPE == AT86RF212)
	 printf("switch power to 10dBm \r\n");
#else
	 printf("switch power to 3dBm \r\n");
#endif
    ...
    tal_pib_set(phyTransmitPower, (pib_value_t *)&temp_var);
	 ...
	 while(0 == pal_sio_rx(SIO_CHANNEL, (uint8_t *)&c, 1))
	 {
	 		pal_task(); /* Handle platform specific tasks, like serial interface */
        tal_task(); /* Handle transceiver specific tasks */
        app_task(); /* Application task */	
...
static void print_result(void)
{
    ...
    else // RX_OP_MODE or PROMISCUOUS_OP_MODE
    {
        ...
        printf("Percent of received frames = %.2f%%; average LQI = %d\r\n", (double)(number_rx_frames/100), (uint8_t)aver_lqi);
    }
}
...
#if 0
/**
 * @brief Sub-menu to get into low power mode
 */
static retval_t set_lwpwr_mode(void)
{
   tal_trx_status_t trx_status;
...
#if (TAL_TYPE == AT86RF231) || (TAL_TYPE == AT86RF212)
	 /*Enable transciver's extended feature */
	 pal_trx_reg_write(RG_TRX_CTRL_1, (pal_trx_reg_read(RG_TRX_CTRL_1) & 0x7F));
#endif
	...
	/* un-initial SPI0  */
	trx_interface_uninit();
	
	/* un-initial SPI1  */
	At25_interface_uninit();
	
	/*  un-initial ADC  */
	adc_uninitialize(AT91C_BASE_ADC);
