...
/**
 * @brief Un-Initializes the transceiver interface
 *
 * This function initializes the transceiver interface.
 * This board uses SPI0.
 */
void trx_interface_uninit(void)
{
    ...
    AT91C_BASE_PIOA->PIO_OER = (MOSI | SCK);
	 AT91C_BASE_PIOA->PIO_IFER = (MISO);
    AT91C_BASE_PIOA->PIO_PER = (MISO | MOSI | SCK);
	 AT91C_BASE_PIOA->PIO_SODR = (MOSI |SCK);

	 AT91C_BASE_PIOA->PIO_OER = TRX_INTERRUPT_PIN;
    AT91C_BASE_PIOA->PIO_PER = TRX_INTERRUPT_PIN;
	 AT91C_BASE_PIOA->PIO_CODR = TRX_INTERRUPT_PIN;

    /* Set SEL as output pin. */
    AT91C_BASE_PIOA->PIO_OER = SEL;
    AT91C_BASE_PIOA->PIO_PER = SEL;
	 AT91C_BASE_PIOA->PIO_SODR = SEL;
	
	 /*  Clear SPI0 MR setting   */
	 AT91C_BASE_SPI_USED->SPI_MR = 0;
	 AT91C_BASE_SPI_USED->SPI_CSR[2] = 0;

    /* The SPI is disabled. */
    AT91C_BASE_SPI_USED->SPI_CR = AT91C_SPI_SPIDIS | AT91C_SPI_SWRST;
}
...
void gpio_init(void)
{
    ...
    AT91C_BASE_PIOA->PIO_OER = SLP_TR;
	 ...
	 /* Disable UART1 RX pull-up.   */
	 AT91C_BASE_PIOA->PIO_PPUDR |= (1L << 5);
}

/**
 * @brief Un-Initializes the GPIO pins
 *
 * This function is used to initialize the port pins used to connect
 * the microcontroller to transceiver.
 */
...
/**
 * @brief Un-Initialize LEDs
 */
void pal_led_uninit(void)
{
    AT91C_BASE_PIOB->PIO_SODR = LED_PINS;
}
...
/**
 * @brief Un-Initializes the ADC controller
 *
 * @param pAdc Pointer to an AT91S_ADC instance
 */
void adc_uninitialize(AT91S_ADC *pAdc)
{
    /* Reset the controller. */
    pAdc->ADC_CR = AT91C_ADC_SWRST;
